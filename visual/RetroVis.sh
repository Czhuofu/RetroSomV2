...
rm -r $datapath/$subject/visual/temp
mkdir $datapath/$subject/visual/temp
echo -e $chr"\t"$cord1"\t"$cord2 > $datapath/$subject/visual/temp/00_cord.bed
...
awk '{printf $1"\t"$2"\t"$3"\t"; for (i=6; i<=NF; ++i) {printf "%s,", $i; if ($i == $NF) printf "\n"}}' \
  ...
  intersectBed -wo \
   -a stdin \
   -b $datapath/$subject/visual/temp/00_cord.bed \
   > $datapath/$subject/visual/temp/01_overlapping_insertions.txt

### Step2: extract the supporting reads ###
$masterpath/visual/MEI_support_reads.pl $PEfile $SRfile $masterpath $TEclass $datapath/$subject/visual 

### Step3: Realign the reads to TEfamily consensus sequence ###
petemp=$datapath/$subject/visual/temp/02_PEsupport.fa
if [ -s "$petemp" ]
then
env TMPDIR=$datapath/$subject exonerate --model affine:local \
   ...
   $petemp \
   $ref | grep "^INFO" \
   > $datapath/$subject/visual/temp/04_PE.alignment
fi

srtemp=$datapath/$subject/visual/temp/03_SRsupport.fa
if [ -s "$srtemp" ]
then
env TMPDIR=$datapath/$subject exonerate --model affine:local \
   --bestn 1 --ryo "INFO: %qi %qal %pi %tS %ti %qab %qae %tab %tae %tas\n" \
   $srtemp \
   $ref | grep "^INFO" \
   > $datapath/$subject/visual/temp/05_SR.alignment
fi

...
my (%nb, %logr, $rf, $i, $count);
    ...
    $rf_file = $path.'/'.$sub.'/retro_v'.$ver.'_'.$strand.'/ALU/'.$sub.'.pe2.pred.summary';
    $lr_file = $path.'/'.$sub.'/retro_v'.$ver.'_'.$strand.'/ALU/'.$sub.'.pe.pred.logR.txt';
    $nb_file = $path.'/'.$sub.'/retro_v'.$ver.'_'.$strand.'/ALU/'.$sub.'.pe.pred.NB.txt';
    $ou_file = $path.'/'.$sub.'/retro_v'.$ver.'_'.$strand.'/ALU/'.$sub.'.pe.prob3.txt';
    %logr = %nb = ();  

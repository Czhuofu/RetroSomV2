...
###########################################
jid7_0=$(echo -e '#!/bin/sh\n'"singularity run -B $outpath,$tmppath,$masterpath,$bam --app 07_combine.lib $sifimagepath  -o $outpath -i $tag -m $masterpath -r $ver -s 0 -l $libs -g $hg" | sbatch -J 1.1.1.Combine $slurm_sc | awk '{print $4}')
jid7_1=$(echo -e '#!/bin/sh\n'"singularity run -B $outpath,$tmppath,$masterpath,$bam --app 07_combine.lib $sifimagepath  -o $outpath -i $tag -m $masterpath -r $ver -s 1 -l $libs -g $hg" | sbatch -J 1.1.2.Combine $slurm_sc | awk '{print $4}')


jid7a0=$(echo -e '#!/bin/sh\n'"singularity run -B $outpath,$tmppath,$masterpath,$bam --app 07a_L1Pairing $sifimagepath  -o $outpath -i $sub -m $masterpath -r $ver -s 0 -n $maxreads" | sbatch -J 1.2.1.L1Pair $slurm_sc --dependency=afterok:$jid7_0:$jid7_1 | awk '{print $4}')
jid7a1=$(echo -e '#!/bin/sh\n'"singularity run -B $outpath,$tmppath,$masterpath,$bam --app 07a_L1Pairing $sifimagepath  -o $outpath -i $sub -m $masterpath -r $ver -s 1 -n $maxreads" | sbatch -J 1.2.2.L1Pair $slurm_sc --dependency=afterok:$jid7_0:$jid7_1 | awk '{print $4}')
jid7b0=$(echo -e '#!/bin/sh\n'"singularity run -B $outpath,$tmppath,$masterpath,$bam --app 07b_AluPairing $sifimagepath  -o $outpath -i $sub -m $masterpath -r $ver -s 0 -n $maxreads" | sbatch -J 1.3.1.AluPair $slurm_sc --dependency=afterok:$jid7_0:$jid7_1 | awk '{print $4}')
jid7b1=$(echo -e '#!/bin/sh\n'"singularity run -B $outpath,$tmppath,$masterpath,$bam --app 07b_AluPairing $sifimagepath  -o $outpath -i $sub -m $masterpath -r $ver -s 1 -n $maxreads" | sbatch -J 1.3.2.AluPair $slurm_sc --dependency=afterok:$jid7_0:$jid7_1 | awk '{print $4}')

########################################
### Step8: Prediction for read pairs ###
########################################
jid8a=$(echo -e '#!/bin/sh\n'"singularity run -B $outpath,$tmppath,$masterpath,$bam --app 08a_L1Predict $sifimagepath  -o $outpath -i $sub -m $masterpath -r $ver -g $hg -p $pcutoff -e $cont" | sbatch -J 2.1.L1Predict  $slurm_sc --dependency=afterok:$jid7a0:$jid7a1 | awk '{print $4}')
jid8b=$(echo -e '#!/bin/sh\n'"singularity run -B $outpath,$tmppath,$masterpath,$bam --app 08b_AluPredict $sifimagepath  -o $outpath -i $sub -m $masterpath -r $ver -g $hg -p $pcutoff -e $cont" | sbatch -J 2.2.AluPredict  $slurm_sc --dependency=afterok:$jid7b0:$jid7b1 | awk '{print $4}')
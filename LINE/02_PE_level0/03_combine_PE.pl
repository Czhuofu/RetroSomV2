...
my $ou1_file = $workfolder.'/'.$sample.'/'.$retro.'_0/LINE/'.$sample.'.pe1.pred.all.txt';
my $ou2_file = $workfolder.'/'.$sample.'/'.$retro.'_0/LINE/'.$sample.'.pe1.pred.summary';
my $ou3_file = $workfolder.'/'.$sample.'/'.$retro.'_1/LINE/'.$sample.'.pe1.pred.all.txt';
my $ou4_file = $workfolder.'/'.$sample.'/'.$retro.'_1/LINE/'.$sample.'.pe1.pred.summary';
my @order = (1,4,8,2,7,6,5,3); 
my ($line, @data);
my (%pred1, %pred2, %pred3, %pred4, %pred5, %pred6, %pred7, %pred8);
my ($i, $pred, $prob, $id, $pos, $neg, $g1, $g2, $g3, $g4, $g5, $g6, $g7, $g8);
...
while ($line = <M1>)
   {
    chomp($line);
    @data = split("\t", $line);
    if (1) # (($data[6] == 1) || ($data[7] == 1)) && ($data[24] == 0) )
       {

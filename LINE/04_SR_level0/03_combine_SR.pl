...
my $ou1_file = $workfolder.'/'.$sample.'/'.$retro.'_0/LINE/'.$sample.'.sr1.pred.all.txt';
my $ou2_file = $workfolder.'/'.$sample.'/'.$retro.'_0/LINE/'.$sample.'.sr1.pred.summary';
my $ou3_file = $workfolder.'/'.$sample.'/'.$retro.'_1/LINE/'.$sample.'.sr1.pred.all.txt';
my $ou4_file = $workfolder.'/'.$sample.'/'.$retro.'_1/LINE/'.$sample.'.sr1.pred.summary';
my @order = (1,2); 
my ($line, @data);
my (%pred1, %pred2);
my ($i, $pred, $id, $pos, $neg, $g1, $g2, $prob);
...
while ($line = <M1>)
   {
    chomp($line);
    @data = split("\t", $line);
    if (1) #  (($data[6] == 1) || ($data[7] == 1)) && ($data[13] == 0) )
       {
